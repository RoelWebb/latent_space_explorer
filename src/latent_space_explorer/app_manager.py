...
# mesh_VARIATION.py is a very slighlty adjusted version of the mesh.py script in the DeepSDF paper implementation written by Park et al.
from latent_space_explorer.utils import mesh_VARIATION as mesh
...
class AppManager(QObject):
    ...
    def reduced_dim_click(self, x: float, y: float, button: int) -> None:
        ...
        if button == 0 or button == 1:  # Clicked left or right
            ...
            if np.all(barycentric_weights >= 0) and np.all(barycentric_weights <= 1):  # Avoid extrapolation
                ...
                print("No code implemented for extrapolation (unable to peform barycentric weighting at selected location)")
    ...
    def linear_interpolate(self, interpolate_percentage: int) -> None:
        ...
        self.shape_reconstructor.reconstruct_shape(self.active_shape_codes[2],
                                                   self.mesh_paths[ShapeType.INTERPOLATE],
                                                   mesh_resolution=self.mesh_res_interpolation)

    def update_barycentric_weighted_codes(self,
                                          shape_idx: int,
                                          barycentric_weights: np.array,
                                          simplex_corner_indeces: np.array) -> None:
    ...
    def normalized_code_channel_update(self, element_idx: int, sign: int) -> None:
        ...
        self.active_shape_codes[0, element_idx] = normalize.normalized_channel_update(self.active_shape_codes[0, element_idx],
                                                                                      self.code_normalize_params[self.code_space],
                                                                                      element_idx,
                                                                                      sign,
                                                                                      self.img_value_step)
    ...
    def set_code_img(self) -> None:
        """Update the image of connected code image widget. When in single shape selection mode,
        set image to normalized latent code in the correct space or for double shape selection set their difference"""
        ...
        if self.visualize_mode is VisualizeShapeMode.SINGLE:
            self.code_img_changed.emit(code_to_img(normalize.normalize(self.active_shape_codes[0], self.code_normalize_params[self.code_space])))
        elif self.visualize_mode is VisualizeShapeMode.DOUBLE:
            self.code_img_changed.emit(code_to_img(np.abs(normalize.normalize(self.active_shape_codes[0], self.code_normalize_params[self.code_space])
                                                          - normalize.normalize(self.active_shape_codes[1], self.code_normalize_params[self.code_space]))))
    ...
    def reconstruct_from_dim_reduction(self) -> None:
        ...
        self.shape_reconstructor.reconstruct_shape(self.active_shape_codes[self.visualize_mode.value],
                                                   self.mesh_paths[ShapeType(self.visualize_mode.value)],
                                                   mesh_resolution=self.mesh_res_selection)

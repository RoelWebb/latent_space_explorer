...
def create_mesh(
    ...
    # Create marching cubes indeces and volume value sample array
    overall_index = torch.arange(0, N ** 3, 1, out=torch.LongTensor())
    samples = torch.zeros(N ** 3, 4)
    temp_samples = torch.zeros(N ** 3, 4)
    ...
    temp_samples[:, 0] = (samples[:, 0] * voxel_size) + voxel_origin[2]
    temp_samples[:, 1] = (samples[:, 1] * voxel_size) + voxel_origin[1]
    temp_samples[:, 2] = (samples[:, 2] * voxel_size) + voxel_origin[0]

    samples[:, 0] = temp_samples[:, 0]
    samples[:, 1] = temp_samples[:, 2]
    samples[:, 2] = temp_samples[:, 1]
